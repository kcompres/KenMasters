...
if($TargetUPN){
  $Users = Get-MsolUser -UserPrincipalName $TargetUPN
} elseif($TargetGroup){
  $GroupId = Get-MsolGroup -SearchString $TargetGroup -MaxResults 1 | Select -ExpandProperty ObjectId
  $Users = (Get-MsolGroupMember -GroupObjectId $GroupId) | Get-MsolUser
} else {
